...
const App = () => {
	...
	//random number to display anecdote 
	const random = () => setSelected(Math.floor(Math.random() * anecdotes.length))

	//vote for the displayed anecdote
	//correct way of updating state stored in complex data structures like arrays 
	//is to make a copy of the state.
	const vote = () => {
		const copy = [...points]
		copy[selected] += 1 //selected anecdote gets +1 vote
		setPoints(copy)
	}
